...
|---------|----------|-----------|--------|----------|
| **[x86@loopback](x86@loopback/)** | x86_64, arm64 (macOS/Linux) | Mock (socketpair) | ✅ Complete | Local development, CI/CD, reference implementation |
...
Harness (parent process)
    ↓ fork() + exec()
x86@loopback Adapter (child process)
    ↓ dlopen()
...
**Complete guide:** [x86@loopback/README.md](x86@loopback/README.md)
...
- **No** → Use **x86@loopback** or **stm32-h7@uart**

**Question 4: Are you benchmarking transport overhead?**
- **Yes** → Use **x86@loopback** (minimal overhead, isolates kernel time)
- **No** → Any adapter is fine

### Comparison Table

| Feature | x86@loopback | jetson-nano@tcp | stm32-h7@uart |
|---------|--------------|-----------------|---------------|
...
| `adapter_name` | string | Adapter identifier (e.g., "x86@loopback") |
...
**x86@loopback:** ~50µs per window (7% overhead for typical kernels)
...
**Yes.** HELLO frame advertises multiple kernels, CONFIG selects one. Phase 1 `x86@loopback` advertises only `noop@f32`, but Phase 1.1 will advertise all 6 kernels.
