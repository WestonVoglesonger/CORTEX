 ...
 * - Pre-generate synthetic datasets for standard configurations (64→2048ch,
 * 160→500Hz)
 ...
 * - Benefits: Fast queries, reproducible benchmarks, scalable to high channel
 * counts
 * - Location: datasets/tools/ directory with generation/benchmarking/query
 * tools
 */
...
static int extract_windows(const eeg_data_t *data, size_t W, size_t H,
  ...
  /* Guard against underflow: if num_samples < W, we can only create at most 1
   * window */
  ...
  test_window_t *windows =
      (test_window_t *)calloc(max_windows, sizeof(test_window_t));
  if (!windows) {
    return -1;
  }
...
static int test_kernel(const char *kernel_name, const test_config_t *config) {
  ...
  for (int i = 0; i < config->max_windows && i < (int)num_windows; i++) {
    ...
    if (!result.passed) {
      fprintf(stderr,
              "  Window %d FAILED: %zu mismatches, "
              "max_abs=%.2e, max_rel=%.2e\n",
              i, result.mismatches, result.max_abs_error, result.max_rel_error);
      failures++;
