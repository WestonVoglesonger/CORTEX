 ...
 * 1. Streams packet-sized deliveries (not windows)
 * 2. Maintains correct real-time cadence (packet_samples/Fs seconds per packet)
 * 3. Handles EOF correctly (rewind and continue)
 * 4. Works with various configurations (different packet_samples, Fs, C)
 * 5. Delivers data in correct order (continuity)
 */
...
static void test_callback(const void *packet_data, size_t packet_samples, void *user_data) {
    ...
    ctx->callback_count++;
    ctx->total_samples_received += packet_samples;

    /* Verify packet size */
    if (packet_samples != ctx->expected_packet_size) {
        fprintf(stderr, "ERROR: Expected %zu samples, got %zu\n",
                ctx->expected_packet_size, packet_samples);
    }

    /* Optionally store received data for validation */
    if (ctx->received_data && ctx->total_samples_received <= ctx->received_data_capacity) {
        size_t offset = ctx->total_samples_received - packet_samples;
        memcpy(ctx->received_data + offset, packet_data, packet_samples * sizeof(float));
    }
}
...
/* Test 1: Verify packet-sized deliveries (not windows) */
static int test_packet_sized_delivery(void) {
    printf("TEST: packet-sized delivery (not windows)\n");

    const char *test_file = "/tmp/cortex_test_packet.dat";
    const uint32_t sample_rate = 160;
    const uint32_t channels = 4;
    const uint32_t packet_samples = 8;
    const size_t total_samples = packet_samples * channels * 10; /* 10 packets */

    create_test_dataset(test_file, total_samples);

    test_context_t ctx = {0};
    ctx.expected_packet_size = packet_samples * channels;
    ctx.expected_period_sec = (double)packet_samples / (double)sample_rate;

    cortex_replayer_config_t config = {0};
    config.dataset_path = test_file;
    config.sample_rate_hz = sample_rate;
    config.channels = channels;
    config.packet_samples = packet_samples;
    ...
    usleep(200000); /* 200ms - should get multiple callbacks @ 50ms intervals */

    cortex_replayer_destroy(replayer);

    /* Verify we got packet-sized deliveries */
    TEST_ASSERT(ctx.callback_count >= 3, "Expected at least 3 callbacks");
    TEST_ASSERT_EQ(ctx.expected_packet_size, packet_samples * channels,
                   "Packet size should equal packet_samples*C");

    printf("  ok  Received %zu callbacks with %zu samples each (packet_samples=%u)\n",
           ctx.callback_count, ctx.expected_packet_size, packet_samples);
...
static int test_timing_cadence(void) {
    ...
    const uint32_t packet_samples = 80;
    const size_t total_samples = packet_samples * channels * 20;

    create_test_dataset(test_file, total_samples);

    test_context_t ctx = {0};
    ctx.expected_packet_size = packet_samples * channels;
    ctx.expected_period_sec = (double)packet_samples / (double)sample_rate; /* 0.5s */
    ...
    TEST_ASSERT_NEAR(ctx.expected_period_sec, avg_period,
                     ...
                     "Average period should match packet_samples/Fs");

    printf("  ok  Timing cadence correct (%.3f s per packet)\n", avg_period);
...
static int test_eof_rewind(void) {
    ...
    const uint32_t packet_samples = 16;
    const size_t packets_in_file = 3;
    const size_t total_samples = packet_samples * channels * packets_in_file;
    ...
    /* File has 3 packets, run long enough to hit EOF and rewind (should loop) */
    usleep(500000); /* 0.5s - should get multiple loops */

    cortex_replayer_destroy(replayer);

    /* Should have gotten more callbacks than packets in file (due to rewind) */
    TEST_ASSERT(ctx.callback_count > packets_in_file,
                "Should loop after EOF via rewind");

    printf("  ok  Got %zu callbacks from %zu packets (looped via rewind)\n",
           ctx.callback_count, packets_in_file);
...
static int test_various_configs(void) {
    ...
    for (size_t i = 0; i < sizeof(configs) / sizeof(configs[0]); i++) {
        ...
        const size_t total_samples = configs[i].packet_samples * configs[i].channels * 5;
        create_test_dataset(test_file, total_samples);

        test_context_t ctx = {0};
        ctx.expected_packet_size = configs[i].packet_samples * configs[i].channels;
        ctx.expected_period_sec = (double)configs[i].packet_samples / (double)configs[i].sample_rate;

        cortex_replayer_config_t config = {0};
        config.dataset_path = test_file;
        config.sample_rate_hz = configs[i].sample_rate;
        config.channels = configs[i].channels;
        config.packet_samples = configs[i].packet_samples;
        ...
        TEST_ASSERT_EQ(ctx.expected_packet_size, configs[i].packet_samples * configs[i].channels,
                       "Packet size mismatch");

        printf("  ok  %s: Fs=%u Hz, C=%u, packet_samples=%u (%.1f ms period)\n",
               configs[i].name,
               configs[i].sample_rate,
               configs[i].channels,
               configs[i].packet_samples,
...
static int test_data_continuity(void) {
    ...
    const size_t total_samples = packet_samples * channels * num_packets;
    ...
    /* Run long enough to get all packets */
    usleep((int)(ctx.expected_period_sec * num_packets * 1000000 * 1.5));
    ...
    printf("  ok  Data continuity verified (%zu samples)\n", ctx.total_samples_received);
...
int main(void) {
    ...
    RUN_TEST(test_packet_sized_delivery);
