...
| Latency decomposition | P5; SE-5,7 | Yes | 3-step latency analysis: `cortex predict` (pre-benchmark Roofline regime classification—compute vs memory bound), `cortex run` (benchmark with PMU telemetry), `cortex decompose` (post-hoc characterization fitting measured PMU data to distribution landmarks, IPC/frequency/stall decomposition, and provenance tracking). Roofline classifies bottleneck regime; decompose quantifies actual overhead. | Innovate |
