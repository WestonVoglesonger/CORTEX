...
| Multi-dtype kernels | P2; SE-3 | Yes | ABI defines FLOAT32, Q15, Q7 as first-class types. 7 of 8 kernels implemented in both f32 and Q15 (welch_psd excluded—requires Q15 FFT). Q15 oracles model exact integer arithmetic chains; all validated C-vs-oracle within quantized tolerances (rtol=1e-3, atol=1e-3). Infrastructure: dtype-aware scheduler buffers, replayer float32→Q15 conversion, dtype in telemetry records. Q7 (int8_t) deferred—same infrastructure applies, but higher quantization noise requires separate validation study. | Implemented |
