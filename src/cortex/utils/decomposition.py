    ...
    if has_pmu:
        ...
        if valid_freq.any():
            ...
            if max_freq_hz > 0 and effective_freq_hz > max_freq_hz * 1.2:
                # PMU cycle counter uses fixed reference clock (e.g., Apple Silicon
                # CNTPCT_EL0), not actual CPU frequency. Detected because measured
                # "frequency" exceeds device spec max by >20%.
